...
for i, n in {3:"Executive Summary",4:"Strengths Highlight",5:"Weak Areas Highlight",6:"Priority Fixes",7:"Visual Severity Indicators",8:"Category Score Breakdown",9:"Industry-Standard Presentation",10:"Certified Export Readiness"}.items():
    add(i, n, 'Executive', 0.5)
...
# C: 21-40
labels_c = ['HTTP 2xx Pages','HTTP 3xx Pages','HTTP 4xx Pages','HTTP 5xx Pages','Redirect Chains','Redirect Loops','Broken Internal Links','Broken External Links','robots.txt Blocked URLs','Meta Robots Blocked URLs','Non-Canonical Pages','Missing Canonical Tags','Incorrect Canonical Tags','Sitemap Missing Pages','Sitemap Not Crawled Pages','Hreflang Errors','Hreflang Conflicts','Pagination Issues','Crawl Depth Distribution','Duplicate Parameter URLs']
for idx, name in enumerate(labels_c, start=21): add(idx, name, 'Crawlability', 1.0)
...
for idx, name in enumerate(labels_d, start=41): add(idx, name, 'OnPage', 1.0)
...
for idx, name in enumerate(labels_e, start=76): add(idx, name, 'Performance', 1.0)
...
for idx, name in enumerate(labels_f, start=97): add(idx, name, 'MobileSecurityIntl', 1.0)
...
for idx, name in enumerate(labels_g, start=151): add(idx, name, 'Competitors', 0.8)
...
for idx, name in enumerate(labels_h, start=168): add(idx, name, 'BrokenLinks', 1.0)
...
for idx, name in enumerate(labels_i, start=181): add(idx, name, 'Opportunities', 0.9)
