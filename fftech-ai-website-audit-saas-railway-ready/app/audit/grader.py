...
"""Standalone page grader (not used by runner).
It analyzes a single HTML document and computes SEO/Performance/Security/Content scores,
returning a nested breakdown suitable for other consumers.
"""
import re
...
def grade_website(html: str, url: str) -> Dict:
    ...
    # 2) Performance (DOM-based heuristics only)
    ...
    # 3) Security (very light heuristics)
    parsed = urlparse(url)
    security_score = 0
    security_issues = []
    ...
    internal_links = [
        ...
        if urlparse(url).netloc in a['href']
    ]
    ...
    total_score = (
        breakdown['seo']['score'] + breakdown['performance']['score'] +
        breakdown['security']['score'] + breakdown['content']['score']
    )
