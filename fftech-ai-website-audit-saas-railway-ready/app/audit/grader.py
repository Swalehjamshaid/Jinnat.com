...
class WebsiteGrader:
    ...
    async def fetch_performance(self, url: str) -> Optional[AuditMetrics]:
        ...
        overall = _estimate_overall_score(ttfb_s, total_time_s, html_kb, scripts, stylesheets, status_code)
    ...
    def _compute_category_scores(
        ...
        s_dom = _score_linear(dom_nodes, good=700.0, bad=5000.0)
        s_js = _score_linear(scripts, good=5.0, bad=40.0)
        s_css = _score_linear(styles, good=3.0, bad=20.0)
        s_img = _score_linear(images, good=10.0, bad=120.0)
        s_size = _score_linear(html_kb, good=200.0, bad=3000.0)
