...
def generate_charts(audit_result: dict) -> str:
    ...
    colors = ['#2E86DE', '#58D68D', '#F5B041', '#EB4D4B']
...
def generate_bar(labels, values, title, filename):
    """
    Generic bar chart generator used by competitor_report.py.
    """
    fig, ax = plt.subplots(figsize=(6, 3))
    ...
    # Dynamic scaling for metrics like LCP (ms) which can exceed 100
    numeric_vals = [v for v in values if isinstance(v, (int, float))]
    upper = max(numeric_vals + [100])
    ax.set_ylim(0, upper * 1.1) # Add 10% padding at the top
...
def export_xlsx(audit_result: dict) -> str:
    """
    Exports raw audit data to an Excel file for data analysis.
    """
    # Flattening nested dictionaries (like breakdown) into single columns
