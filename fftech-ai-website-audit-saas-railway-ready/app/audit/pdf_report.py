...
Filename: pdf_report.py (updated, fixed)
Railway-Ready | Single-File Executable | No External Dependencies (except standard libs & common py libs)

What's new in this version
--------------------------
• Fix: Python syntax error replaced (!csp_missing → not csp_missing).
• Added generate_audit_pdf(url, output_path, logo_path=None) for runner integration.
...
def safe_get(url: str, headers: Optional[dict] = None, timeout: int = 15,
             allow_redirects: bool = True, method: str = "GET"):
    try:
...
def get_ssl_cert_expiry(domain: str, port: int = 443) -> Optional[str]:
    """Return SSL notAfter string if available."""
    try:
...
class SaaSWebAuditor:
    ...
    def run_audit(self) -> Dict[str, Any]:
        try:
            ...
            if robots_txt_ok and 'sitemap' in (robots_txt_res.text or '').lower():
                sitemap_present = True
            ...
            impact_notes = [{**iss, "impact": estimate_business_impact(iss['cat'], iss['sev'])} for iss in top5]

            ip = get_ip(self.domain)
            ssl_status = "Valid (expiry: %s)" % get_ssl_cert_expiry(self.domain) if ssl_ok else "N/A"
            ...
            a11y_details = {
                ...
                "missing_aria_labels": f"Heuristic: {'Some present' if aria_present else 'Few/None detected'}",
            ...
            # Collect broken links (both internal and externalAbsolutely, Khan — here’s the **complete and fixed `pdf_report.py`** that resolves the `SyntaxError` and **adds a proper `generate_audit_pdf(url, output_path, logo_path=None)`** function so your runner can import it:

- ✅ Fixes `!csp_missing` → `not csp_missing`
- ✅ Provides a **drop‑in** `generate_audit_pdf(...)` used by `app.audit.runner`
- ✅ Keeps all the sections you requested (Cover, Executive Summary with charts, Overview, SEO, Performance, Security, Accessibility, UX, Broken Links, Tracking)
- ✅ Railway-ready, resilient timeouts, graceful N/A fallbacks

> **Path:** `app/audit/pdf_report.py`  
> **Runner import:** `from app.audit.pdf_report import generate_audit_pdf`

---

### ✅ Complete, Updated `pdf_report.py`
...
What's new in this version
--------------------------
• Fixed Python syntax error (! -> not) causing container crash
• Exposes generate_audit_pdf(url, output_path, logo_path=None) for runner integration
• Expanded audit engine to populate all sections requested (SEO, Performance, Security, Accessibility, UX, Links, Tracking)
• Safer network timeouts, graceful fallbacks, and clear N/A markers where lab/field APIs are required
• Executive Summary with radar + bar chart, risk level, top-5 critical issues, and impact estimates
• Cover page with optional company logo, confidentiality notice, and QR to target
• Website overview (domain, IP, SSL, redirects, page size, total requests, etc.)
