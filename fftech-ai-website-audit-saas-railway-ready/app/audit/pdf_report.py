...
- Smarter fallbacks and optional hide-empty-rows mode
"""
...
def _parse_time_to_seconds(val: Any) -> Optional[float]:
    """
    Accepts floats, "120 ms", "1.8 s", "1.8s", "120ms" etc. Returns seconds.
    """
    try:
...
def _filter_rows(rows: List[List[Any]], hide: bool) -> List[List[Any]]:
    """
    Optionally remove rows whose value column is 'N/A'. Assumes 2-col rows or [label, value, ...].
    """
    if not hide: return rows
    out = []
    for r in rows:
        if len(r) == 0: continue
        # Keep header rows or any row where at least one non-header cell is not 'N/A'
        if isinstance(r[0], str) and r[0] in ("Metric","Header","Issue","Element","Device"):
...
def _page_cover(audit: Dict, styles) -> List[Any]:
    ...
    if isinstance(logo_path,str) and logo_path:
        ...
        except: story.append(Spacer(1,10))
...
def _page_overview(audit: Dict, styles) -> List[Any]:
    ...
    # Security header coverage (if available)
...
def _page_performance(audit: Dict, styles) -> List[Any]:
    ...
    extras={}; extras.update(_safe_get(audit,"core_web_vitals",default={}) or {})
    ...
    # Charts: grouped bars (Current vs Ideal)
    story.append(Spacer(1,8))
...
def _page_security(audit: Dict, styles) -> List[Any]:
    ...
    hdrs={"content_security_policy":False,"x_frame_options":False,"x_content_type_options":False,
    ...
    for label,key in [("Content-Security-Policy","content_security_policy"),
    ...
    # Risk heat map from issues list
    issues=_safe_get(audit,"issues",default=[])
